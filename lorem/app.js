const text = [
  ...
  form.addEventListener('submit', (e)=>{
    ...
    const value =  parseInt(amount.value); // capturamos el valor del input
    const random = Math.floor(Math.random() * text.length); // seleccion al azar de los textos
    
    //Si el valor esta por fuera del rango, imprime un texto al azar
    if(isNaN(value) || value < 0 || value > 9){
        result.innerHTML = ` <p class="result"> ${text[random]} </p>`
    }else{
        let tempText = text.slice(0, value); //extraemos el numero de textos de acuerdo al rango
        
        //retornamos el mapa de textos
